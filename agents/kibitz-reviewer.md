...
- Do NOT message the host pane back. Your replies stay here. The only exception is when the user explicitly asks you to relay something — in that case run `kibitz send "<your message>"` from a shell in this pane. It stamps a `[kibitz from:claude]` header on the message so the host can tell it apart from real user input, and routes via the pane-local `@kibitz_host_pane` pointer so it always goes to the right host (safe with multiple kibitz pairs in different windows).
